...
- [~] タスク: 在庫設定・更新用の `InventoryController` と基本APIの実装
- [ ] タスク: 在庫管理の基本機能に関する機能テスト（Feature Test）の作成
- [ ] タスク: Conductor - 手動検証 '在庫管理基盤の構築' (workflow.mdのプロトコルに準拠)

## フェーズ 2: MRP計算エンジンのコアロジック実装
- [ ] タスク: 計算エンジンを管理する `MrpService` の作成
- [ ] タスク: 再帰的な所要量計算（在庫考慮なし）のユニットテスト作成
- [ ] タスク: `MrpService` での基本的な再帰展開の実装
- [ ] タスク: 正味所要量計算（在庫引当含む）のユニットテスト作成
- [ ] タスク: `MrpService` への在庫引当ロジックの実装（在庫がある場合は優先使用し、不足分のみ下位展開する）
- [ ] タスク: Conductor - 手動検証 'MRP計算エンジンのコアロジック実装' (workflow.mdのプロトコルに準拠)
