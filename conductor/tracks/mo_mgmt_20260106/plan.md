...
- [~] タスク: テスト用の Factory 作成
- [ ] タスク: リレーションとステータス遷移定数のユニットテスト作成
- [ ] タスク: Conductor - 手動検証 'データベーススキーマとモデルの実装' (workflow.mdのプロトコルに準拠)

## フェーズ 2: 製造指示 生成ロジック（Service層）の実装
- [ ] タスク: `MoService` クラスの作成
- [ ] タスク: 生産計画から製造指示を生成し、BOMをスナップショット保存するロジックのユニットテスト作成
- [ ] タスク: `MoService` での生成・BOMコピーロジックの実装
- [ ] タスク: 自動採番（MO番号）機能の実装
- [ ] タスク: Conductor - 手動検証 '製造指示 生成ロジックの実装' (workflow.mdのプロトコルに準拠)
