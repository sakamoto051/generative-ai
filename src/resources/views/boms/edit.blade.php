...
<div class="max-w-4xl mx-auto">
  <!-- ヘッダー -->
  <div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-900">BOM編集</h2>
    <p class="mt-1 text-sm text-gray-600">部品表情報を編集します</p>
  </div>

  <!-- 編集フォーム -->
  <div class="bg-white shadow-sm sm:rounded-lg">
    <form action="{{ route('boms.update', $bom) }}" method="POST" class="p-6 space-y-6">
      @csrf
      @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="md:col-span-2">
            ...
            <select name="material_id" id="material_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('material_id') border-red-500 @enderror">
              ...
              <option value="{{ $material->id }}" {{ old('material_id', $bom->material_id) == $material->id ? 'selected' : '' }} data-unit="{{ $material->unit }}">
          ...
          <div>
            ...
            <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $bom->quantity) }}" step="0.01" min="0.01" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('quantity') border-red-500 @enderror">
            @error('quantity')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            <input type="text" name="unit" id="unit" value="{{ old('unit', $bom->unit) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('unit') border-red-500 @enderror">
            @error('unit')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            <input type="number" name="yield_rate" id="yield_rate" value="{{ old('yield_rate', $bom->yield_rate) }}" step="0.1" min="0" max="100" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('yield_rate') border-red-500 @enderror">
            @error('yield_rate')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          ...
          <div>
            ...
            <input type="number" name="sequence" id="sequence" value="{{ old('sequence', $bom->sequence) }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('sequence') border-red-500 @enderror">
            @error('sequence')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          ...
          <div class="md:col-span-2">
            ...
            <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('notes') border-red-500 @enderror">{{ old('notes', $bom->notes) }}</textarea>
            @error('notes')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
